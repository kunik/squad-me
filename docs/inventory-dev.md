...
Updated: 2026-07-17T22:07:00Z (GHA deploy + Access smoke green)
...
| Access CI smoke | **Live.** Service token `squad-me-gha-smoke` + policy `Allow CI smoke`; `CF_ACCESS_CLIENT_*` on GitHub `cloud-dev` |
| GitHub env | `cloud-dev`: `CLOUDFLARE_ACCOUNT_ID`, `CLOUDFLARE_API_TOKEN` (`squad-me-ci`), `CF_ACCESS_CLIENT_ID`, `CF_ACCESS_CLIENT_SECRET` **set**. Deploy + smoke green via GHA |
...
- [x] `squad-me-ci` Zone Workers Routes + Access scopes (GHA deploy + smoke bootstrap)
