...
- [ ] `wrangler secret put` for Production identity keys: Turnstile + Twilio Verify (+ confirm Gateway/session). See `docs/provision.md` § Identity / auth secrets — do not put until values confirmed
- [ ] Budget alerts on Gateway + Twilio before public OTP traffic
- [x] GitHub Environment `production` created with `CLOUDFLARE_ACCOUNT_ID` (no required reviewers; gate = manual `workflow_dispatch`; optional `commit_sha`, default latest `main`)
