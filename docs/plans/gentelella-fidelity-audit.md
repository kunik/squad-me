...
- [ ] **Membership UX** — замінити `.profile-form__toggle-block` на Gentelella `.toggle` / `toggle-row` де можливо, або `form_advanced.html` patterns. *(Відкрито — наступний кандидат у Profile widgets.)*

- [ ] **DateField** — стилізувати popover ближче до Gentelella dropdown/calendar (border-radius, shadow tokens уже близькі — потрібен markup alignment). *(Відкрито — polish.)*
...
- [x] **SHELL-001/002/003** — scrollbar gutter + lock compensation; brand sidebar backdrop; rail footer rules scoped to ≥769px.
