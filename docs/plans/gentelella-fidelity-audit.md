...
- [ ] **Profile page layout** — перебудувати під `profile.html`: `col-4-8` identity+form, notifications як `.toggle` rows, security/sessions як окремі cards; aside anchor nav або прибрати, або перенести в лівий summary card як inline links (ближче до ring-box / nav у template). *(Blocked on product remarks — see backlog above.)*
