...
4. **Auth content vs `login.html`**
   ...
   - Замість цього: phone-based форма, `HintPanel` всередині card; без lang/theme у `AuthLayout` (лише на Home).
   - **Exit footer:** `AuthExitLink` — signed-in → `/profile` («До профілю»), guest → `/` («На головну»); не per-page хардкод (AUTH-006).
...
**Розміщення controls — відхилення.** Gentelella: `.tb-btn.theme-toggle` у topbar. Squad Me: `ThemeSwitch` у Profile aside prefs / `GuestUtilities` (Home only; auth forms hide lang/theme).
...
- [x] **Home** — intentional non-Gentelella screen; shares `GuestBrand` with auth, keeps `GuestUtilities` (lang/theme) on Home only (not `landing.html`).
...
- [x] **SHELL-001/002/003** — scrollbar gutter + lock compensation (drawer + modal); shared `--overlay-scrim` (sidebar + modal + template cmdk/drawer); rail footer rules scoped to ≥769px.
