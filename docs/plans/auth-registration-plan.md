...
| `POST` | `/api/auth/phone/change` | (auth) proof → update phone |
...
- [App.tsx](../../src/client/App.tsx): маршрути `/login`, `/register`, `/forgot-password` (мінімум)
...
login/register/forgot-password, `/profile` і legacy alias-ів). Login після
успіху теж веде на `/profile`, якщо крок ще pending (пріоритетніше за
`?next=`). Header «Профіль» → `/profile`.
