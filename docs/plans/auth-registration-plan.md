...
**Статус:** Phase 1–4 **Done**; Phase 5 **Done as designed (stub scope v1)**;
Phase 0 **Pending (owner)**. Короткий STATUS:
[`auth-registration-STATUS.md`](./auth-registration-STATUS.md).

Реалізовано в `src/worker/identity/` + client auth UI; покрито тестами
(`npm test`). **Відкрито лише для власника:** Phase 0 secrets/alerts
(Turnstile widget keys, Twilio Verify, budget alerts —
`docs/provision.md` § "Identity / auth secrets"; Telegram Gateway уже live).
**Scrypt:** локальний `npm run bench:scrypt` Done; CPU-bench на задеплоєних
Workers = **won't do until traffic ramp**.
...
server-driven через `onboardingStep` на `/profile`.
**Phase 5 v1:** UI shell + email collection + SMS-as-connected;
email OTP confirm / Telegram connect APIs / preferred-channel persistence /
push APIs = **won't do v1** (майбутній notifications план / KB).
...
**Password hashing параметри:** scrypt `N=2^15, r=8, p=1` — локальний sample
`npm run bench:scrypt` **Done**. Deployed Workers CPU bench =
**won't do until traffic ramp** (тоді переглянути `N/r/p` під CPU budget).
Якщо budget не влазить — fallback на WebCrypto PBKDF2-SHA-256 ≥600 000
iterations (OWASP 2023 мінімум), але це саме fallback, не primary вибір.
...
### Phase 0 — Secrets & docs — **Pending (owner)**

Акаунти Gateway + Twilio Verify; secrets local/dev/prod; оновити `.dev.vars.example` (+ `OTP_SINK_MODE=log` за зразком `NOTIFICATION_SINK_MODE`), provision/inventory; нотатка як тестувати під Dev Access; **budget alert** на Gateway/Twilio витрати (Cloudflare notification / provider dashboard alert) — налаштувати одразу, доки лімітів на OTP ще немає. Чекбокси gate лишаються unchecked, доки власник не виконає кроки — не вигадувати secrets.

### Phase 1 — Schema + password + session — **Done**

Міграція `0002_identity.sql` (з індексом на `auth_challenges`); scrypt
hash/verify helpers; session create/load/revoke (в т.ч.
revoke-all-except-current для reset-флоу); `me` / `logout`; unit-тести за
патерном [concurrency.test.ts](../../src/worker/concurrency.test.ts)
(vitest pool workers). Локальний `npm run bench:scrypt` Done; Workers CPU
bench = **won't do until traffic ramp**.

### Phase 2 — Phone OTP API — **Code done** (live OTP → Phase 0)
...
### Phase 3 — Register / login / reset + UI — **Done**
...
### Phase 5 — Notify hooks (light) — **Done as designed (stub scope v1)**
...
Email: `POST /api/auth/account/email` (collection). SMS — `phoneE164` з auth
як connected.

**Won't do v1** (майбутній notifications план / KB
`products/match-platform/specs/notifications.md`):
- email OTP confirm / `emailConnected`
- Telegram connect APIs / `telegramUserId`
- preferred-channel persistence на бекенді (дефолт SMS)
- Web Push / `push_subscriptions` send APIs
...
confirm, Telegram Bot linking API, preferred-channel persistence і Web Push
APIs — **won't do v1** (окремий notifications план; UI «Мої сповіщення» —
radio-ряди + connect shells; live лишається collection `Account.email` +
SMS phone з auth). URL-кроки для pre-auth phone/OTP/password (option 1) —
...
Phone-only passwordless; email/OIDC/Firebase; OTP на кожен login; повний
Bot/Web Push send; email OTP confirm; Telegram connect APIs;
preferred-channel persistence; локальні UA SMS/Viber; auth Durable Object;
deployed Workers scrypt CPU bench (до traffic ramp).
